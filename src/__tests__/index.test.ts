  ...
  actualReactNative.NativeModules.MindboxSdk = {
    ...
    executeAsyncOperation: jest.fn(
      (operationSystemName: string, operationBodyString: string) =>
        new Promise((resolve, reject) => {
          if (operationSystemName && typeof operationSystemName === 'string' && operationSystemName.length > 0 && operationBodyString && typeof operationBodyString === 'string' && operationBodyString.length > 0) {
            resolve(true)
          } else {
            reject(new Error('Error'))
          }
    ...
    executeSyncOperation: jest.fn(
      (operationSystemName: string, operationBodyString: string) =>
        new Promise((resolve, reject) => {
          if (operationSystemName && typeof operationSystemName === 'string' && operationBodyString && typeof operationBodyString === 'string') {
            ...
            } else if (operationBodyString !== JSON.stringify({ recommendation: { limit: '10' } })) {
              resolve(
                JSON.stringify({
                  type: 'MindboxError',
                  data: {
                    status: 'ProtocolError',
                    errorMessage: '/recommendation/limit: "ururu" не является корректным целым числом. Поле должно содержать целое число не превышающее 2147483647.',
                  ...
                  recommendations: [
                    {
                      ...
                      description: "Made from sweat-wicking fabric, the NikeCourt Advantage RF Men's Tennis Polo helps keep you cool and dry when the match heats up.",
                      ...
                      pictureUrl: 'https://demoshop.mindbox.cloud/upload/iblock/462/4622e4f88b4fe73063404f01aac9c4b0.jpg',
                      price: 1275.0,
                      oldPrice: 2550.0,
                      category: 'T-shirts',
                    },
...
describe('Testing Mindbox RN SDK', () => {
  const initializationData = {
    ...
    endpointId: Platform.OS === 'ios' ? 'pushok-rn-ios-sandbox' : 'pushok-rn-android-sandbox',
  ...
  describe('Testing Mindbox RN SDK native methods', () => {
    ...
    describe('Testing initialize method', () => {
      it('throws errors when no paylaod passed or payload is not a JSON string', async () => {
        ...
        await expect(MindboxSdk.initialize('notJSONString')).rejects.toThrow('Unexpected token')
      })
      ...
      it('throws error when no valid payload data was passed', async () => {
        ...
        await expect(MindboxSdk.initialize(notValidInitializationData)).rejects.toThrow('Error')
      })
      ...
      it('resolves successfully with string payload passed', async () => {
        ...
        await expect(MindboxSdk.initialize(payloadString)).resolves.toBeTruthy()
      })
    })
    ...
    describe('Testing executeAsyncOperation method', () => {
      it('throws error when no payload passed', async () => {
        ...
        await expect(MindboxSdk.executeAsyncOperation('', asyncOperationBodyString)).rejects.toThrow('Error')

        await expect(MindboxSdk.executeAsyncOperation(asyncOperationSystemName, '')).rejects.toThrow('Error')
      })

      it('throws error when non string payload passed', async () => {
        expect.assertions(2)

        await expect(MindboxSdk.executeAsyncOperation(123, asyncOperationBodyString)).rejects.toThrow('Error')

        await expect(MindboxSdk.executeAsyncOperation(asyncOperationSystemName, 123)).rejects.toThrow('Error')
      })

      it('resolves successfully with string payload passed', async () => {
        expect.assertions(1)

        await expect(MindboxSdk.executeAsyncOperation(asyncOperationSystemName, asyncOperationBodyString)).resolves.toBeTruthy()
      })
    })
    ...
    describe('Testing executeSyncOperation method', () => {
      it('throws error when no payload passed', async () => {
        ...
        await expect(MindboxSdk.executeSyncOperation('', syncOperationBodyString)).rejects.toThrow('Error')

        await expect(MindboxSdk.executeSyncOperation(syncOperationSystemName, '')).rejects.toThrow('Error')
      })

      it('throws error when non string payload passed', async () => {
        expect.assertions(2)

        await expect(MindboxSdk.executeSyncOperation(123, syncOperationBodyString)).rejects.toThrow('Error')

        await expect(MindboxSdk.executeSyncOperation(syncOperationSystemName, 123)).rejects.toThrow('Error')
      })

      it('resolves with ProtocolError with wrong operation name', async () => {
        expect.assertions(1)

        await expect(MindboxSdk.executeSyncOperation('WrongOperationName', syncOperationBodyString)).resolves.toBe(
          JSON.stringify({
            type: 'MindboxError',
            data: {
              status: 'ProtocolError',
      ...
      it('resolves successfully with string payload passed', async () => {
        ...
        await expect(MindboxSdk.executeSyncOperation(syncOperationSystemName, syncOperationBodyString)).resolves.toBe(
          JSON.stringify({
            status: 'Success',
            recommendations: [
              {
  ...
  describe('Testing Mindbox RN SDK public interface', () => {
    ...
    it('initialize method resolves successfully', async () => {
      ...
      await expect(MindboxSdk.initialize(initializationData)).resolves.toBeUndefined()
